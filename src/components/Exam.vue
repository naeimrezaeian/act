...
export default {
  ...
  methods: {
    ...mapActions(['subtestQuestions', 'setTime', 'updateQuestionPointer', 'finishExam']),

    updateQuestion() {
      console.log({method:'updateQuestion',data:this.getCurrentPointer, data2:this.getQuestion(this.getCurrentPointer)})
      ...
      if (this.getNextQuestion) {
        console.log({method:'updateQuestion',data:'not last'})
        this.LastQuestion = false
      } else {
        console.log({method:'updateQuestion',data:'is last'})
        this.LastQuestion = true
      }
      console.log({method:'updateQuestion',data:'done'})
    ...
    async send() {
      console.log({ method: 'send', data: this.currentSubtestId })
  ...
  async created() {
    console.log({ method: 'Exam.Created', data: this.currentStateData })
    console.log({ method: 'Exam.Created', data: this.currentSubtestId })
    ...
    this.$store.subscribe(async (mutation) => {
      switch (mutation.type) {
        case 'updateTimeleft':
          ...
          if (this.timerFinish) {
            console.log({ method: 'updateTimeleft', data: this.currentSubtestId })
            await this.finishExam(this.currentSubtestId)
          }
          break
        case 'updateSendAnswer':
          console.log({method:'updateSendAnswer',data:this.getNextQuestion})
          if (this.getNextQuestion) {
